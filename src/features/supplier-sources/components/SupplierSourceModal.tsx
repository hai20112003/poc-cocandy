...
interface SupplierSourceModalProps {
  open: boolean
  onClose: () => void
  supplierSource?: SupplierSource | null
  onSave: (data: Omit<SupplierSource, 'id'>) => void
}
...
export function SupplierSourceModal({ open, onClose, supplierSource, onSave }: SupplierSourceModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setName(supplierSource?.name ?? '')
      setCode(supplierSource?.code ?? '')
      setErrors({})
    }
  }, [open, supplierSource])
  ...
  function validate(): boolean {
    const e: FormErrors = {}
    if (!name.trim()) e.name = 'Tên supplier-source là bắt buộc'
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={supplierSource ? 'Chỉnh sửa Supplier-source' : 'Thêm Supplier-source mới'}
    >
      <div className="space-y-4">
        <div>
          <label className="block text-[11px] font-medium text-slate-700 dark:text-slate-300 mb-1.5">
            Tên supplier-source <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            value={name}
            ...
            placeholder="Nhập tên supplier-source..."
            className={cn(
