    ...
    <Modal open={open} onOpenChange={onOpenChange} title={`Chi tiết phiếu nhập kho: ${data.code}`}>
      <div className="space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 rounded border">
          <div>
            <div className="text-xs text-slate-600">Mã GRN</div>
          ...
          <div>
            <div className="text-xs text-slate-600">Ngày nhận</div>
        ...
        {data.transportationUnit && (
          <div className="grid grid-cols-2 gap-3 p-3 bg-blue-50 rounded border border-blue-200">
            <div>
              <div className="text-xs text-blue-600 font-semibold">Đơn vị vận chuyển</div>
              <div className="font-medium text-slate-800">{data.transportationUnit}</div>
            </div>
            <div>
              <div className="text-xs text-blue-600 font-semibold">Mã vận đơn</div>
        ...
        {/* Items Table */}
        <div>
          <h4 className="font-semibold text-sm mb-2">Chi tiết hàng nhận ({data.items.length} dòng)</h4>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-100 border-b">
                  ...
                  <th className="px-2 py-2 text-center font-semibold">SL đặt</th>
                  <th className="px-2 py-2 text-center font-semibold">Nhận</th>
                  <th className="px-2 py-2 text-center font-semibold">Chấp nhận</th>
                  <th className="px-2 py-2 text-center font-semibold">Từ chối</th>
                  <th className="px-2 py-2 text-left font-semibold">Lô / Vị trí</th>
                  <th className="px-2 py-2 text-left font-semibold">QC Note</th>
                </tr>
              </thead>
              <tbody>
                {data.items.map((item, idx) => (
                  <tr key={item.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="px-2 py-2 text-center">{item.orderedQuantity}</td>
                    <td className="px-2 py-2 text-center font-medium">{item.receivedQuantity}</td>
                    <td className="px-2 py-2 text-center font-medium text-green-700">{item.acceptedQuantity}</td>
                    <td className="px-2 py-2 text-center font-medium text-red-700">{item.rejectedQuantity}</td>
                    <td className="px-2 py-2 text-xs text-slate-600">
                      <div>{item.lotNumber}</div>
                      <div>{item.warehouseLocation}</div>
                    </td>
                    <td className="px-2 py-2 text-xs text-slate-600">{item.qcNote}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-3 gap-3 p-3 bg-slate-50 rounded border">
          <div>
            <div className="text-xs text-slate-600">Tổng nhận</div>
            <div className="font-bold text-lg text-slate-800">{data.totalReceived}</div>
          </div>
          <div>
            <div className="text-xs text-green-600">Chấp nhận</div>
